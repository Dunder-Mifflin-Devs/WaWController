...
const {
  searchOMDB,
  randomOMDB,
} = require("../../src/microServices/MediaService/mediaControllers/mediaControllers");
const {
  omdbSearch,
} = require("../../src/microServices/MediaService/utils/fetchResults");
const {
  getRandomWord,
...
describe("Media Controller Tests", () => {
  ...
  test("results of a title search are formatted correctly", async () => {
    ...
    expect(await searchOMDB(data.omdbTitleRequest)).toEqual(
      data.omdbTitleExpected
    );
  });
  ...
  test("results of an id search are formatted correctly", async () => {
    ...
    expect(await searchOMDB(data.omdbIdRequest)).toEqual(data.omdbIdExpected);
  });

  test("results of a failed search are formatted correctly", async () => {
    omdbSearch.mockImplementation((params) => data.omdbErrorResponse);

    expect(await searchOMDB(data.omdbErrorRequest)).toEqual(
      data.omdbErrorExpected
    );
  });
  ...
  test("results of a random search of 5 are formatted correctly", async () => {
    ...
    expect(await randomOMDB(data.omdbRandomRequest5)).toEqual(
      data.omdbRandomExpected5
    );
  });
  ...
  test("results of a random search of 10 are formatted correctly", async () => {
    ...
    expect(await randomOMDB(data.omdbRandomRequest10)).toEqual(
      data.omdbRandomExpected10
    );
  });
  ...
  test("results of a random search of 15 are formatted correctly", async () => {
    ...
    expect(await randomOMDB(data.omdbRandomRequest15)).toEqual(
      data.omdbRandomExpected15
    );
  });
  ...
  test("results of an error in a random search are formatted correctly", async () => {
    ...
    expect(await randomOMDB(data.omdbRandomErrorRequest)).toEqual(
      data.omdbRandomErrorExpected
    );
  });
